class Solution(object):
    def pacificAtlantic(self, matrix):
        ...
        # 1 => reachable
        # 0 => unreachable
        toPacific = [[ 1 for x in range(columns)]] + [[1] + [-1 for x in range(columns-1)] for x in range(rows-1)]
        toAtlantic = [[-1 for x in range(columns - 1)] + [1] for y in range(rows-1)] + [[1 for z in range(columns)]]
        ...
        def dfs(pos, toOcean):
            ...
            toOcean[i][j] = "pending"
            for n in nexts:
                if toOcean[n[0]][n[1]] == "pending":
                    continue
                if dfs(n, toOcean) == True:
                    toOcean[i][j] = 1
                    return True
            toOcean[i][j] = 0
            return False
        ...
        for i in range(rows):
            for j in range(columns):
                toA, toP = False, False
                if toPacific[i][j] == -1:
                    toP = dfs((i, j), toPacific)
                elif toPacific[i][j] == 1:
                    toP = True
                if toAtlantic[i][j] == -1:
                    toA = dfs((i, j), toAtlantic)
                elif toAtlantic[i][j] == 1:
                    toA = True
                if toA and toP:
                    ans.append((i, j))
        # return ans
        return dfs((3,27), toAtlantic)

ary = [[1, 2, 2, 3, 5], [3, 2,3,4,4], [2,4,5,3,1], [6,7,1,4,5], [5,1,1,2,4]]
s = Solution()
# print(s.pacificAtlantic(ary))
